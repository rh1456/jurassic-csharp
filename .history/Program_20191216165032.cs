...
namespace jurassic_csharp
{
  class Program
  {
    ...
    static void ReleaseDino()
    {
      ...
      var dinoName = Console.ReadLine();
      var updateDino = Db.Dinosaurs.FirstOrDefault(dino => dino.Name.ToLower() == dinoName.ToLower());
      updateDino.EnclosureNumber = 0;

      Db.SaveChanges();
    }
    static void NewRandoDino()
    {
      string[] names = { "Frank", "Sally", "Joe", "Mark", "Celeste", "Hank", "Chandler", "Michael" };
      string[] diet = { "carnivore", "herbivore" };
      var dino = new Dinosaur();
      Random random = new Random();
      dino.Name = names[random.Next(0, 11)];  //Returns non-negative random integer from 0-11
      dino.DietType = diet[random.Next(0, 2)];
      dino.DateAcquired = DateTime.Now;
      dino.Weight = random.Next(0, 15000);
      Console.WriteLine($"Hatched! Welcome {dino.Name} a {dino.DietType} weighing {dino.Weight} pounds");
      Db.Dinosaurs.Add(dino);
      Db.SaveChanges();
    }
    static void SheepFunction()
    {
      DisplayListOfDinosaur(Db.Dinosaurs.Where(dino => dino.DietType == "carnivore").OrderBy(dino => dino.Weight).Take(3));
    }
